...
def main(winstyle=0):
    ...
    fullscreen = winstyle == pg.FULLSCREEN
    ...
    while player.alive():
        ...
        # handle player input
        if joystick:
          direction = int(round(joystick.get_axis( 0 ), 0))
        else:
          direction = keystate[pg.K_RIGHT] - keystate[pg.K_LEFT]        
        player.move(direction)

        if joystick:
          button = joystick.get_button( A_BUTTON )
          firing = button == 1
        else:
...
if __name__ == "__main__":
    fullscreen = True
    winstyle = pg.FULLSCREEN if fullscreen else 0  # |FULLSCREEN
    main(winstyle)
    pg.quit()
